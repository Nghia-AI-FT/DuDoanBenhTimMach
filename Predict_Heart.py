...
Combine_Heart = pickle.load(open('Combine_trained_model.pkl', 'rb'))
...
if (selected == 'Combine Heart Prediction'):
    ...
    # code for Prediction
    Combine_heart_diagnosis = ''
    
    # creating a button for Prediction
    
    if st.button('Combine Test Result'):
        heart_combi_prediction = Combine_Heart.predict([[age, sex, cp, trestbps, chol, fbs, restecg,thalach,exang,oldpeak,slope,ca,thal]])                          
        
        if (heart_combi_prediction[0] == 1):
          Combine_heart_diagnosis = 'The person is having heart disease'
        else:
          Combine_heart_diagnosis = 'The person does not have any heart disease'
        
    st.success(Combine_heart_diagnosis)

print(type(Combine_Heart))
...
if (selected == 'Improve Heart Prediction'):
    ...
    if st.button('Improve Test Result'):
        ImPr_heart = Improve_Heart.predict([[age, sex, cp, trestbps, chol, fbs, restecg,thalach,exang,oldpeak,slope,ca,thal]])                          
